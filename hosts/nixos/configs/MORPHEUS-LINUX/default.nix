...
in
{
  ...
  boot = {
      ...
      if enableXanmod then
        lib.mkForce
          (pkgs.unstable.linuxPackagesFor
            (pkgs.unstable.linuxKernel.kernels.linux_xanmod_latest.override {
              argsOverride = rec {
                ...
                src = pkgs.fetchurl {
                  url = "https://github.com/xanmod/linux/archive/refs/tags/${version}-${suffix}.tar.gz";
                  sha256 = "sha256-EPPq9HJzehRaYe7mTDnU2bM7pUKQ8/OqC4uIhVFNAO8=";
                };
              };
            }))
      else
    ...
    extraModulePackages = lib.mkForce [ pkgs.unstable.linuxPackages_xanmod_latest.zfs pkgs.unstable.evdi ];
    ...
    initrd.systemd.services = {
      ...
      create-needed-for-boot-dirs = {
        after = pkgs.lib.mkForce [ "zfs-import-zdata.service" "zfs-import-zroot.service" ];
        wants = pkgs.lib.mkForce [ "zfs-import-zdata.service" "zfs-import-zroot.service" ];
      };
    };
  };
